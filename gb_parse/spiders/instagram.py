...
class InstagramSpider(scrapy.Spider):
    ...
    def tag_parse(self, response):
        ...
        # def img_url(tag):
        #     for i in tag:
        #         yield i['node']['display_url']

        yield InstaTag(
            # img=img_url(tag['edge_hashtag_to_media']['edges']),
    ...
    def get_post_item(edges):
        for node in edges:
            yield InstaPost(date_parse=datetime.datetime.utcnow(), data=node["node"], img=node['node']['display_url'])
